...
function RespawnTimer()
	...
	ESX.TriggerServerCallback('esx_ambulance:ambulancecount', function(count)
		CountAmbulance = tonumber(count)
		if CountAmbulance <= 0 then
			NoAmbulance()
		elseif CountAmbulance > 0 then
			HasAmbulance()
		end
    end)  
...
function HasAmbulance()
	local timer = Config.RespawnDelayAfterRPDeath
    ESX.TriggerServerCallback('esx_ambulancejob:getDeathTime', function(dead_time)
        if dead_time > 0 then
            timer = dead_time
        else
            TriggerServerEvent('esx_ambulancejob:setDeathTime', timer)
        end
    end)
      ...
      while timer <= 0 and IsDead do
        ...
        if IsControlPressed(0,  Keys['E']) then
          ...
          ESX.TriggerServerCallback('esx_ambulancejob:removeItemsAfterRPDeath', function()
            ...
            TriggerServerEvent('esx:updateLastPosition', Config.Zones.HospitalInteriorInside1.Pos)
            TriggerServerEvent('esx_ambulancejob:setDeathTime', 0)
            RespawnPed(GetPlayerPed(-1), Config.Zones.HospitalInteriorInside1.Pos)
            StopScreenEffect('DeathFailOut')
            DoScreenFadeIn(800)
          end)
...
function NoAmbulance()
    local timer = Config.RespawnDelayNoAmbulance
    ...
    Citizen.CreateThread(function()
      ...
      while timer > 0 and IsDead do
        ...
        seconds = stringsplit(raw_seconds-(minutes*60), ".")[1]
          ...
          SetTextOutline()
          SetTextEntry("STRING")
          AddTextComponentString('No Ambulance '.._U('please_wait') .. minutes .. _U('minutes') .. seconds .. _U('seconds')..' and pay 1000$')
          SetTextCentre(true)
          DrawText(0.5, 0.8)
          timer = timer - 15
          Citizen.Wait(0)
      ...
      while timer <= 0 and IsDead do
        ...
        SetTextOutline()
        SetTextEntry("STRING")
        AddTextComponentString(_U('press_respawn').. ' and pay 1000$')
        SetTextCentre(true)
        DrawText(0.5, 0.8)
        Citizen.Wait(0)
		...
		TriggerServerEvent('removemoneyautorevive', GetPlayerServerId(PlayerId()))
        ...
        ESX.UI.Menu.Open(
          'default', GetCurrentResourceName(), 'citizen_interaction',
          {
            ...
            elements = {
			  ...
			  {label = ('ชุบตัวเอง'), value = 'reviveself'},			  
            }
          },
          function(data, menu)
			...
			if data.current.value == 'reviveself' then
			TriggerServerEvent('esx_ambulancejob:revive', GetPlayerServerId(PlayerId()))
            end
...
function OpenCloakroomMenu()
	...
	-- if Config.EnableNonFreemodePeds and PlayerData.job.grade_name == 'chief_doctor' then
		-- table.insert(elements, { label = 'ชุดสกิน Lana สำหรับหมอ pp❤', value = 'lana' })
		-- table.insert(elements, { label = 'ชุดสกิน Sam สำหรับหมอ pp❤', value = 'sam' })
	-- end
  ...
  ESX.UI.Menu.Open(
	...
	if data.current.value == 'lana' then
		local model = 'lana'
        RequestModel(model)
...
function OpenVehicleSpawnerMenu()
  ...
  if Config.EnableSocietyOwnedVehicles then
    ...
    if PlayerData.job.grade_name == 'ambulance' then 
       table.insert(elements, { label = 'รถกู้ชีพ', value = 'pol718' })
	   table.insert(elements, { label = 'รถเบนซืกู้ชีพ', value = 'polrs' })
    end
    ...
    if PlayerData.job.grade_name == 'doctor_Ex' then
        table.insert(elements, { label = 'รถกู้ชีพ', value = 'pol718' })
	   table.insert(elements, { label = 'รถเบนซืกู้ชีพ', value = 'polrs' })
    end

    if PlayerData.job.grade_name == 'doctor_Pro' then
	    table.insert(elements, { label = 'รถกู้ชีพ', value = 'pol718' })
	   table.insert(elements, { label = 'รถเบนซืกู้ชีพ', value = 'ems_gs1200' })
    end

    if PlayerData.job.grade_name == 'doctor_cheif' then
	...
	if PlayerData.job.grade_name == 'chief_doctor' then
	    table.insert(elements, { label = 'รถกู้ชีพ', value = 'pol718' })
	   table.insert(elements, { label = 'ฮอกู้ชีพ', value = 'supervolito' })
    end
    if PlayerData.job.grade_name == 'boss' then
	    table.insert(elements, { label = 'รถกู้ชีพ', value = 'protopolice' })
		table.insert(elements, { label = 'ฮอกู้ชีพ', value = 'supervolito' })
		table.insert(elements, { label = 'RSInterceptor_EMS', value = 'polrs' })
		table.insert(elements, { label = 'porsche_EMS', value = 'pol718' })
		table.insert(elements, { label = 'BMWGS1200_EMS', value = 'ems_gs1200' })
    end	
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if Config.EnablePlayerManagement then
          TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), GetPlayerServerId(PlayerId()), _U('fine_total') .. label, amount)
        else
...
Citizen.CreateThread(function()
  while true do
    ...
    if CurrentAction ~= nil then
      ...
      if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and (GetGameTimer() - GUI.Time) > 150 and PlayerData.job.name == 'ambulance' then
    ...
    if IsControlPressed(0,  Keys['F6']) and PlayerData.job ~= nil and (GetGameTimer() - GUI.Time) > 150 and PlayerData.job.name == 'ambulance' then
